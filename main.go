...
func test2WithSTM(data *Data) {
	...
	for i := 0; i < 10001; i++ {
		...
		t1 := MySTM.NewT().
			...
			dCell1 := t.ReadT(cell1)  // read data from memory cell - reads are transactional operations
			dinCell := dCell1.([]int) // extract data to required format
			fmt.Println("dinCell = ", dinCell)
			return true
		}).
			Done()
		fmt.Println(t1)
